...
class FtpServer:
    ...
    '''
    def __init__(self, port, port_allowed, pasv_allowed):
        logger.log('ftpServer initialised.')
        self.port = int(port)
        self.port_allowed = port_allowed
        self.pasv_allowed = pasv_allowed
        self.c = None
    ...
    def port_command(self):
        try:
            ...
            if not self.port_allowed:
                logger.log('Port is not allowed.')
            elif self.logged_in_check():
    ...
    def pasv_command(self):
        try:
            ...
            if not self.pasv_allowed:
                logger.log('Pasv is not allowed.')
            elif self.logged_in_check():
                self.data_receiving_socket = FtpServer.create_new_socket()
...
def main():
    ...
    port_mode = config['server_config']['port_mode']
    pasv_mode = config['server_config']['pasv_mode']

    if port_mode == 'NO':
        port_mode = False
        logger.log('Port Mode not allowed.')
    elif port_mode == 'YES':
        port_mode = True
        logger.log('Port Mode allowed.')
    else:
        logger.log('Should Not reach here. Something is wrong')
        print('')

    if pasv_mode == 'NO':
        pasv_mode = False
        logger.log('Passive Mode not allowed.')
    elif port_mode == 'YES':
        pasv_mode = True
        logger.log('Passive Mode allowed.')
    else:
        logger.log('Should Not reach here. Something is wrong')
        print('')
    ...
    port = int(sys.argv[2])
    ftpServer = FtpServer(port, port_mode, pasv_mode)
